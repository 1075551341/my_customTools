...
import type { VideoTranscodeConfig, ImgTranscodeConfig, AnimTranscodeConfig, DocumentTranscodeConfig } from '../types'
 ...
 * 队列处理器映射表
 */
const queueProcessors: Map<string, (job: Queue.Job<TranscodeJobData>) => Promise<unknown>> = new Map()
  ...
  imageQueue.process(concurrency, async (job: Queue.Job<TranscodeJobData>) => {
    ...
    try {
      ...
      const encoderConfig: ImgTranscodeConfig = {
        outputFormat: (task.outputFormat || 'jpg') as ImgTranscodeConfig['outputFormat'],
...
function startDocumentWorker(): void {
  ...
  documentQueue.process(concurrency, async (job: Queue.Job<TranscodeJobData>) => {
    const { taskId, type, inputPath, outputPath, config: transcodeConfig } = job.data
    ...
    try {
      ...
      // 获取转码配置
      const encoderConfig = transcodeConfig as unknown as DocumentTranscodeConfig
 ...
 * @param data - 完成数据
 */
function pushCompleted(userId: string, data: { taskId: string; outputSize: number; format: string; duration?: number }): void {
 ...
 * @param data - 失败数据
 */
function pushFailed(userId: string, data: { taskId: string; errorMsg: string }): void {
